...
class StorageLevel private(
  ...
  override def toString: String = {
    ...
    val far = if (farLevel > 0) "FAR" + farLevel else ""

    val output =
      Seq(disk, memory, heap, deserialize, s"$replication replicas", s"$far").filter(_.nonEmpty)
    s"StorageLevel(${output.mkString(", ")})"
  }

  override def hashCode(): Int = toInt * 41 + replication + _far * 640
...
object StorageLevel {
  ...
  val FAR = new StorageLevel(true, true, false, true, 1, 1)
  ...
  def apply(
      ...
      replication: Int,
      far: Int): StorageLevel = {
    getCachedStorageLevel(
      new StorageLevel(useDisk, useMemory, useOffHeap, deserialized, replication, far))
  }
  ...
  def apply(
      ...
      deserialized: Boolean,
      replication: Int = 1,
      far: Int = 0): StorageLevel = {
    getCachedStorageLevel(
      new StorageLevel(useDisk, useMemory, false, deserialized, replication, far))
  }
