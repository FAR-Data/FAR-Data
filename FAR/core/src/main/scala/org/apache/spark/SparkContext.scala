...
class SparkContext(config: SparkConf) extends Logging {
  ...
  def dagSchedulerForDebugging: Option[DAGScheduler] = Some(_dagScheduler)
  ...
  private[spark] def unpersistRDD(rddId: Int, blocking: Boolean = true) {
    ...
    env.farManager.RDDunpersisted(rddId)
   ...
   * Implemented by FAR
   */
  private[spark] def enableFAR(): Unit = {
    env.farManager.enable = true
  }
   ...
   * Implemented by FAR
   */
  private[spark] def disableFAR(): Unit = {
    env.farManager.enable = false
  }

  /**
   * Unpersist a Block from memory and/or disk storage
   *
   * Implemented by FAR
   */
  def unpersistBlock(blockId: BlockId) {
    env.blockManager.master.removeBlockAsync(blockId)
  }
