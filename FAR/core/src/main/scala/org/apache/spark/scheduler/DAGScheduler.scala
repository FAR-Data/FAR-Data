...
private[spark] class DAGScheduler(
   ...
   */
  private[scheduler] val cacheLocs = new HashMap[Int, IndexedSeq[Seq[TaskLocation]]]
  ...
  private[scheduler] def clearCacheLocs(): Unit = cacheLocs.synchronized {
    cacheLocs.clear()
  }
  ...
  def createShuffleMapStage(shuffleDep: ShuffleDependency[_, _, _], jobId: Int): ShuffleMapStage = {
    ...
    FARManager.get.shuffleMapStageCreated(rdd, stage)
  ...
  private def createResultStage(
    ...
    stageIdToStage(id) = stage
    updateJobIdStageIdMaps(jobId, stage)
    FARManager.get.resultStageCreated(rdd, stage, jobId)
    stage
  }
  ...
  private[scheduler] def resubmitFailedStages() {
    if (failedStages.size > 0) {
      ...
      FARManager.get.failedStagesResubmitting(failedStagesCopy.sortBy(_.firstJobId))

      for (stage <- failedStagesCopy.sortBy(_.firstJobId)) {
        val activeJob = jobIdToActiveJob(stage.firstJobId)
        submitStage(stage)
      }
    }
  }
...
//    farManager.ecsDoubleCheck(job)
    ...
    FARManager.get.activeJobCreated(job)
...
//    farManager.ecsDoubleCheck(job)
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing.isEmpty) {
          ...
          FARManager.get.stageSubmitting(stage)
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    FARManager.get.taskSubmitting(tasks, stage)
  ...
  private[scheduler] def handleTaskCompletion(event: CompletionEvent) {
    ...
    event.reason match {
      case Success =>
        ...
        FARManager.get.taskSucceed(task, stage)
  ...
  private def markStageAsFinished(
    ...
    FARManager.get.stageFinished(stage)
