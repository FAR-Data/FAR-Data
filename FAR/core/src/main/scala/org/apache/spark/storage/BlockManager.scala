...
private[spark] class BlockManager(
  ...
  private[storage] override def dropFromMemory[T: ClassTag](
    ...
    if (blockIsRemoved) {
      blockIsUpdated = true
      // Implemented by FAR
      master.getFARFeedbackAfterEviction(blockId)
    } else {
  ...
  // Implemented by FAR
  private[storage] override def putIntoMemory[T: ClassTag](blockId: BlockId): Unit = {
    val startTime = System.currentTimeMillis()
    master.getFARFeedbackAfterPuttingIntoMemory(blockId)
  }
